...
import {
    adminAccessTokenValid,
    adminRefreshTokenValid,
    testerAccessTokenValid,
    testerRefreshTokenValid,
} from "./tokens";
    ...
    test("T1.1: Returns the list of all the users", (done) => {
        User.insertMany([
            ...
            request(app)
                ...
                .set(
                    "Cookie",
                    `accessToken=${adminAccessTokenValid}; refreshToken=${adminRefreshTokenValid}`
                )
                .then((response) => {
                    const expectedArray = [
                        {
    ...
    test("T1.2: Returns a 401 error if called by a user who is not an Admin", (done) => {
        User.insertMany([
            ...
            request(app)
                ...
                .set(
                    "Cookie",
                    `accessToken=${testerAccessTokenValid}; refreshToken=${testerRefreshTokenValid}`
                )
                .then((response) => {
...
describe("3: createGroup", () => {
    test("T3.1: Returns information about the created group. The user calling the function is inserted among the members", (done) => {
        User.insertMany([
            ...
            const groupInfo = {
                name: "group",
                memberEmails: ["tester@test.com", "tester2@test.com"],
            };
            request(app)
                .post("/api/groups")
                .set(
                ...
                .then((response) => {
                    expect(response.status).toBe(200);
                    expect(response.body.data.group).toHaveProperty(
                        "name",
                        "group"
                    );
    ...
    test("T3.4: Returns a 400 error if all the passed emails are either in a group or do not exist", (done) => {
        User.insertMany([
            ...
            Group.create({
                ...
                const groupInfo = {
                    name: "Group2",
                    memberEmails: ["tester2@test.com", "admin@email.com"],
                };
                request(app)
                    .post("/api/groups")
                    .set(
    ...
    test("T4.1: Returns information about all the groups", (done) => {
        User.insertMany([
            ...
            const users1 = await User.find({
                email: { $in: ["tester@test.com", "tester2@test.com"] },
            });
            ...
            Group.insertMany([
                ...
                request(app)
                    ...
                    .then((response) => {
                        ...
                        expect(response.body.data[1].members[0]).toHaveProperty(
                            "email",
                            "tester3@test.com"
                        );
                        done();
            ...
            Group.insertMany([
                ...
                request(app)
                    ...
                    .then((response) => {
                        ...
                        expect(response.body.data.group.members).toHaveLength(
                            2
                        );
                        response.body.data.group.members.forEach((m) => {
                            const isEmailIn = [
                        ...
                        response.body.data.membersNotFound.forEach((m) => {
                            const stringArray = ["tester4@test.com"].includes(
                                m
                            );
                            const objectArray = m.email
                                ? m.email === "tester4@test.com"
                                : false;
                            const verified = stringArray || objectArray;
                            expect(verified).toBe(true);
                        });
                        response.body.data.alreadyInGroup.forEach((m) => {
                            const stringArray = ["tester3@test.com"].includes(
                                m
                            );
                            const objectArray = m.email
                                ? m.email === "tester3@test.com"
                                : false;
                            const verified = stringArray || objectArray;
                            expect(verified).toBe(true);
                        });
            ...
            Group.insertMany([
                ...
                request(app)
                    ...
                    .then((response) => {
                        ...
                        expect(
                            response.body.data.group.members[0]
                        ).toHaveProperty("email", "tester@test.com");
                        response.body.data.membersNotFound.forEach((m) => {
                            const stringArray = ["tester4@test.com"].includes(
                                m
                            );
                            ...
                            .then(() => {
                                request(app)
                                    ...
                                    .then(async (response) => {
                                        ...
                                        expect(
                                            response.body.data.deletedFromGroup
                                        ).toBe(true);
                                        done();
                                    })
                                    .catch((err) => done(err));
